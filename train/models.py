...
class User(db.Model, UserMixin):
	...
	email = db.Column(db.String(120), unique=True, nullable=False)
	password = db.Column(db.String(60), nullable=False)
	passengers = db.relationship('Passenger', backref='passengers', lazy=True)
...
class Passenger(db.Model):
	...
	ticket = db.relationship('Ticket',backref='ticket',lazy=True,uselist=False)  #uselist=False implies one-to-one relationship
	def __repr__(self):
...
class Train(self):
	...
	source_id = db.Column(db.Integer,db.ForeignKey('station.station_id'),nullable=False)
	destination_id = db.Column(db.Integer,db.ForeignKey('station.station_id'),nullable=False)
